Este repositório funciona como o hub oficial da documentação do projeto **CIMOB** e da organização do grupo **VORTEK**. Aqui fica reunido tudo que a galera precisa pra acompanhar o andamento do projeto: apresentações, relatórios, guias, materiais de apoio e qualquer info importante pra manter o time alinhado.

A ideia é centralizar tudo num lugar só, facilitando tanto o desenvolvimento quanto a manutenção e futuras apresentações. Assim, qualquer membro do grupo consegue acessar rapidamente o que precisa, sem ficar perdido procurando arquivos soltos por aí.

<img width="875" height="397" alt="Screenshot from 2025-09-07 17-31-51" src="https://github.com/user-attachments/assets/208cf0f1-9ff5-480d-9fbb-05483b83b7e6" />

<br><br>

---

## 📌 Informações da Sprint 1
<details>
<summary>Clique para expandir</summary>

### Detalhes da Sprint
- **Descrição**: documentação inicial da sprint.

### 🔗 Links Úteis
- [Apresentação V1](https://github.com/Vortek-API/DocsRepo/blob/main/SPRT-1.pptx)
- [Protótipo de Telas V1](https://github.com/Vortek-API/DocsRepo/blob/main/CIMOB%20-%20V1.pdf)

### Modelagem V1
<img alt="Modelagem da Sprint 1" src="https://github.com/user-attachments/assets/ca4ab10e-928b-4347-abf5-4a08738d91c6" width="800"/>

</details>

---

## 📌 Informações da Sprint 2
<details>
<summary>Clique para expandir</summary>

###  Detalhes da Sprint
- **Descrição**: documentação da segunda sprint.

### 🔗 Links Úteis
- [Apresentação V2](https://github.com/Vortek-API/cimob-docs/blob/main/CIMOB%20-%20V2.pptx)
- [Protótipo de Telas V2](https://github.com/Vortek-API/DocsRepo/blob/main/CIMOB%20-%20V1.pdf)

### Modelagem V2
<img alt="Modelagem da Sprint 2" src="https://github.com/user-attachments/assets/ca4ab10e-928b-4347-abf5-4a08738d91c6" width="800"/>

</details>

---

## 📌 Informações da Sprint 3
<details>
<summary>Clique para expandir</summary>

###  Detalhes da Sprint
- **Descrição**: documentação da terceira sprint.

### 🔗 Links Úteis
- [Apresentação V2](https://github.com/Vortek-API/cimob-docs/blob/main/CIMOB%20-%20V2.pptx)
- [Protótipo de Telas V2](https://github.com/Vortek-API/DocsRepo/blob/main/CIMOB%20-%20V1.pdf)
- [Guia de Instalação](https://github.com/Vortek-API/cimob-docs/blob/main/Guia%20de%20Instala%C3%A7%C3%A3o.pdf)

### Modelagem V3
<img alt="Modelagem da Sprint 3" src="https://github.com/user-attachments/assets/ca4ab10e-928b-4347-abf5-4a08738d91c6" width="800"/>

</details>

---
